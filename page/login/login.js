    ...
    loginFunc: function(){
        ...
        Ajax.post( 
            Config.host+'/mobile/login/login', 
            {
                ...
                if(data.status == 0){
                    //var mobile = AsyncStorage.getItem('mobile');
                    AsyncStorage.setItem('mobile', data.data+','+data.qrcode, function(){
                        _navigator.pop();
                    });
                }
...
//        )
    ...
    render: function(){
        return (
            <View style={{flex:1}}>
                <Header navigator={_navigator} route={_route} title="登陆" right="reg"/>
